...
static char *import_program = NULL;
...
static void get_instance_id()
{
  char   hostname[HOST_NAME_MAX+1];
  struct addrinfo *aill;
  int    rc;

  if( 0 == gethostname( hostname, HOST_NAME_MAX ) )
  {
    if( 0 == ( rc = getaddrinfo( hostname, NULL, NULL, &aill ) ) )
    {
      instance_id = ((struct sockaddr_in *)(aill->ai_addr))->sin_addr.s_addr;
      freeaddrinfo( aill );
    }
    else
    {
      lerr("Exiting due to failure in getaddrinfo(): %s (%d)",
           gai_strerror( rc ), rc );
      exit(EXIT_FAILURE);
    }
  }
  ...
  {
    lerr("Exiting due to failure in gethostname(): %s", strerror(errno));
    exit(EXIT_FAILURE);
  }
}
...
static int import_objects_and_status(char *cfg, char *cache, char *status)
{
	...
	asprintf(&cmd, "%s --nagios-cfg=%s --cache=%s --instance=%lu "
			 "--db-name=%s --db-user=%s --db-pass=%s --db-host=%s",
			 import_program, cfg, cache, instance_id,
