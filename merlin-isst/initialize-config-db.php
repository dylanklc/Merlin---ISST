...
function usage($msg = false)
{
        ...
        echo "--running    Modify running tables\n";
        echo "--staging    Modify staging tables\n";
        exit(1);
}
...
for ($i = 1; $i < $argc; $i++) {
        ...
        if ($optpos !== false) {
                ...
                $opt = $argc[++$i];
        } elseif (! preg_match('/dry-run|running|staging/', $arg)) {
        ...
        switch ($arg) {
         ...
         case 'staging': $imp->env = ENV_STAGING; break;
         case 'running': $imp->env = ENV_RUNNING; break;
         default:
...
if (($imp->env != ENV_STAGING) && ($imp->env != ENV_RUNNING)) {
    usage("Specify either --running or --staging on CLI (default: --staging)");
}
